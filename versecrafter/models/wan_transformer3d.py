...
from videox_fun.dist import (get_sequence_parallel_rank,
...
from videox_fun.models.cache_utils import TeaCache
...
def _prepare_sequence_tensors(x, seq_len, sp_world_size):
    """
    Helper function to prepare sequence tensors by padding and concatenating.
    Extracts the common logic for handling grid_sizes, seq_lens, and padding.
    
    Args:
        x (List[Tensor]): List of sequence tensors
        seq_len (int): Target sequence length
        sp_world_size (int): Sequence parallel world size
    
    Returns:
        Tuple: (concatenated_x, seq_lens) where seq_lens is a tensor of original sequence lengths
    """
...
def _process_teacache_skip_logic(teacache, e0, t, cond_flag):
    """
    Helper function to process TeaCache skip logic.
    Extracts the common logic for computing skip flags based on modulated input.
    
    Args:
        teacache: TeaCache instance
        e0 (Tensor): Time embedding of shape [B, 6, C] or [B, C]
        t (Tensor): Timesteps
        cond_flag (bool): Whether this is conditional forward pass
    
    Returns:
        bool: Whether to perform calculation (should_calc)
    """
    if cond_flag:
        if t.dim() != 1:
            modulated_inp = e0[:, -1, :]
        ...
        skip_flag = teacache.cnt < teacache.num_skip_start_steps
        if skip_flag:
            should_calc = True
            teacache.accumulated_rel_l1_distance = 0
        else:
            rel_l1_distance = teacache.compute_rel_l1_distance(
                teacache.previous_modulated_input, modulated_inp
            )
            teacache.accumulated_rel_l1_distance += teacache.rescale_func(rel_l1_distance)
            
            if teacache.accumulated_rel_l1_distance < teacache.rel_l1_thresh:
                should_calc = False
            else:
                should_calc = True
                teacache.accumulated_rel_l1_distance = 0
...
def _process_blocks_forward(blocks, x, e0, seq_lens, grid_sizes, freqs, context,
                            context_lens, dtype, t, gradient_checkpointing):
    """
...
class WanLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        ...
        Args:
            x(Tensor): Shape [B, L, C]
        """
        # DeepSpeed may keep LayerNorm parameters in fp32 even when the activation
        # comes in as bf16/fp16. Run the normalization in fp32 (including affine
        # params) and cast the result back to preserve mixed-precision safety.
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        if add_control_adapter:
            self.control_adapter = SimpleAdapter(in_dim_control_adapter, dim, kernel_size=patch_size[1:], 
                stride=patch_size[1:], downscale_factor=downscale_factor_control_adapter)
        else:
    ...
    def enable_teacache(
        ...
        self.teacache = TeaCache(
            coefficients, num_steps, rel_l1_thresh=rel_l1_thresh, 
            num_skip_start_steps=num_skip_start_steps, offload=offload
        )
    ...
    def enable_riflex(
        ...
        self.freqs = torch.cat(
            [
                get_1d_rotary_pos_embed_riflex(1024, self.d - 4 * (self.d // 6), 
                    use_real=False, k=k, L_test=L_test, L_test_scale=L_test_scale),
    ...
    def forward(
        ...
        if subject_ref is not None:
            ...
            grid_sizes = torch.stack(
                [
                    torch.tensor([u[0] + subject_ref_frames, u[1], u[2]])
                    for u in grid_sizes
                ]
            ).to(grid_sizes.device)
        ...
        if self.teacache is not None:
            if not self.should_calc:
                ...
                x = _process_blocks_forward(
                    self.blocks, x, e0, seq_lens, grid_sizes, self.freqs, context,
                    context_lens, dtype, t, self.gradient_checkpointing
                )
    ...
    def from_pretrained(
        ...
        if model.state_dict()['patch_embedding.weight'].size() != state_dict['patch_embedding.weight'].size():
            model_patch_weight = model.state_dict()['patch_embedding.weight']
            state_patch_weight = state_dict['patch_embedding.weight']
            model_patch_weight[:, :state_patch_weight.size()[1], :, :] = \
                state_patch_weight[:, :model_patch_weight.size()[1], :, :]

...
def _export_clip_ffmpeg(in_path, start_frame, end_frame, fps, width, height, out_path):
    ...
    Frame-accurate cutting with select/setpts.
    """
    ...
    # Probe source video metadata for validation.
    probe_cmd = [
        "ffprobe", "-v", "error",
        "-select_streams", "v:0",
    ...
    try:
        ...
        w, h, rate, nb_frames = result.stdout.strip().split(',')
        src_fps = eval(rate)  # Parse fractional frame rates such as "60/1".
        total_frames = int(nb_frames)
        if e > total_frames:
            raise ValueError(f"end_frame ({e}) exceeds video total frames ({total_frames})")
    except subprocess.CalledProcessError as ex:
        print(f"ffprobe failed: {ex.stderr}")
        raise
    except ValueError as ex:
        print(f"Frame validation failed: {str(ex)}")
        raise

    # Select frame range with the select filter only.
    vf = f"select='between(n,{s},{e-1})'"
    
    cmd = [
        "ffmpeg", "-hide_banner",
        "-loglevel", "warning",  # Use warning level for richer diagnostics.
        "-nostdin",
        "-i", in_path,
        "-vf", vf,
        "-vsync", "vfr",
        "-c:v", "libx264",
        "-pix_fmt", "yuv420p",
        "-r", f"{src_fps}",  # Keep source fps from ffprobe.
        "-an", "-y", out_path,
    ]
    
    # Print full command for easier debugging.
    print(f"Running ffmpeg command: {' '.join(cmd)}")
    
    try:
        # Use subprocess.Popen to capture detailed stderr output.
...
def _visualize_detection_results(image_source, boxes, masks, labels, confidences, prompt, video_path):
    ...
    debug_dir = "./debug_visualization_spatialvid"
...
def _classify_person_by_ar_edge(box_xyxy: np.ndarray, img_h: int, img_w: int) -> str:
    """Classify one person by AR+Edge rules: return 'SMALL' | 'BIG' | 'BIG_INCOMPLETE'.
...
def _is_valid_by_gsam2_with_visualization(
    ...
    Returns: (is_valid: bool, detection_results: dict | None)
    """
    ...
    # NMS
    # print(f"Object num: {input_boxes.shape[0]}")
    if input_boxes.shape[0] > 0:
        ...
        labels = []
    # print(f"Object num after NMS: {input_boxes.shape[0]}")
    ...
    # 4) Branch A (person): AR+Edge rules + count constraints.
    if check_person_completeness:
        classes = []  # 'SMALL' | 'BIG' | 'BIG_INCOMPLETE'
        for i in range(len(input_boxes)):
            c = _classify_person_by_ar_edge(input_boxes[i], h, w)
            classes.append(c)
...
def _is_window_valid_by_gsam2(first_frame_path):
    """
    - Boxes count must be in (0, 6].
    - All mask area percentages must be in [0, 20]%.
    Return True/False.
    """
    # Load image and run GroundingDINO.
    ...
    # Count check.
    if input_boxes.shape[0] == 0 or input_boxes.shape[0] > 6:
        return False

    # Run SAM2 masks and apply area-percentage filtering.
    image_predictor.set_image(image_source)
    masks, scores, logits = image_predictor.predict(
        point_coords=None,
        point_labels=None,
    ...
    # Require all objects to have area percentages within [0, 20]%.
    for area_pct in mask_area_percentages:
        if not (0.0 <= area_pct <= 20.0):
            return False
    return True
...
def main(scenedetect_csv, video_root, output_clip_dir, output_csv, num_parts=1, part_idx=0):
    ...
    for row_idx, row in tqdm(ts_df.iterrows(), total=len(ts_df), desc="Scanning timestamps"):
        ...
        for t in ts_list:
            start_fr = int(float(t[0]))
            end_fr = int(float(t[1]))
            fps = int(float(t[2]))
            ...
            while i <= last_start:
                ...
                is_valid = is_video_frame_valid(frame_rgb)

                if is_valid:
                    out_name = f"{video_id}_{i:07d}_{i+SEG_LEN:07d}.mp4"
                    out_path = os.path.join(output_clip_dir, out_name)
                    ...
                    if not skip_export:
                        try:
                            ...
                            if os.path.exists(out_path):
                                # Validate file size (files below 500KB are treated as invalid).
                                file_size = os.path.getsize(out_path)
                                if file_size < 500 * 1024:  # 500KB
                                    print(f"Generated file too small ({file_size} bytes), removing: {out_path}")
                                    os.remove(out_path)
                                    export_success = False
                                else:
                                    try:
                                        # Validate integrity with decord.
                                        test_vr = VideoReader(out_path, ctx=cpu(0))
                                        if len(test_vr) != SEG_LEN:  # Strictly require exact frame count.
                                            print(f"Generated file has wrong number of frames ({len(test_vr)} != {SEG_LEN}), removing: {out_path}")
                                            os.remove(out_path)
                                            export_success = False
                                        else:
                                            print(f"Exported clip successfully: {out_path}")
                                    except Exception as e:
                                        print(f"Generated file is corrupt, removing: {out_path}")
                                        os.remove(out_path)
                                        export_success = False
                            else:
                                print(f"Export failed - file not found: {out_path}")
                                export_success = False
                                
                        except subprocess.CalledProcessError as e:
                            print(f"ffmpeg failed: {video_path} [{i}-{i+SEG_LEN}]: {e}")
                            export_success = False
                            # Ensure cleanup of partially generated files.
                            if os.path.exists(out_path):
                                os.remove(out_path)
...
if __name__ == '__main__':
    # ========================= Tunable Person Rule Parameters =========================

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch VerseCrafter inference on SpatialVID with comparison grid output")
    parser.add_argument("--csv", default="path/to/spatialvid_test_split.csv", help="CSV path containing clip metadata")
    parser.add_argument("--data-dir", default="/path/to/SpatialVID-HQ/SpatialVid/HQ", help="SpatialVID clip root")
    parser.add_argument("--annotation-dir", default="/path/to/SpatialVID-HQ/clip_annotation", help="Rendered map root")
    parser.add_argument("--output-dir", default="samples/batch_inference/spatialvid", help="Output directory")
    ...
    parser.add_argument("--ulysses-degree", type=int, default=4, help="Ulysses degree")
...
def main() -> None:
    ...
    for idx, (_, row) in enumerate(current_part.iterrows()):
        ...
        try:
            ...
            required_files = {
                "gt_video": data_dir / f"{crowd_density}_clip" / Path(clip_path).name,
                "background_RGB": annotation_root / clip_stem / "background_RGB.mp4",
                "background_depth": annotation_root / clip_stem / "background_depth.mp4",
                "3D_gaussian_RGB": annotation_root / clip_stem / "3D_gaussian_RGB.mp4",
                "3D_gaussian_depth": annotation_root / clip_stem / "3D_gaussian_depth.mp4",
                "merged_mask": annotation_root / clip_stem / "merged_mask.mp4",
            }

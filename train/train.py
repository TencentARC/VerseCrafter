...
# This script lives in `<repo_root>/train/`, so the repository root is one level up.
repo_root = os.path.dirname(os.path.dirname(current_file_path))
project_roots = [os.path.dirname(current_file_path), repo_root, os.path.dirname(repo_root)]
for project_root in project_roots:
    sys.path.insert(0, project_root) if project_root not in sys.path else None

# Resolve `videox_fun` to the pristine upstream package in third_party/VideoX-Fun.
# Inserted at the front so it takes precedence over any local copy of videox_fun.
videox_fun_upstream_path = os.path.join(repo_root, "third_party", "VideoX-Fun")
if videox_fun_upstream_path not in sys.path:
    sys.path.insert(0, videox_fun_upstream_path)
...
from videox_fun.data.dataset_image_video import (ImageVideoControlDataset,
                                                 ...
                                                 process_pose_file,
                                                 process_pose_params)
from versecrafter.data import (AspectRatioBatchImageVideoSampler,
                               ImageVideoSampler,
                               VideoControlDataset_sekai_spatialvid)
...
from videox_fun.utils.utils import (get_image_to_video_latent,
                                    save_videos_grid)
from versecrafter.utils import get_video_to_video_latent
...
def log_validation(vae, text_encoder, tokenizer, transformer3d, args, config, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_unwrapped = accelerator.unwrap_model(transformer3d)
        transformer3d_unwrapped.eval()

        pipeline = WanVerseCrafterPipeline(
            vae=accelerator.unwrap_model(vae).to(weight_dtype), 
            text_encoder=accelerator.unwrap_model(text_encoder),
            tokenizer=tokenizer,
            transformer=transformer3d_unwrapped,
            scheduler=scheduler,
        )
        ...
        for i in range(len(args.validation_prompts)):
            with torch.no_grad():
                with torch.autocast("cuda", dtype=weight_dtype):
                    ...
                    # Check if validation_paths[i] is a directory (multi-control case) or a file (single control case)
                    validation_id = args.validation_paths[i]
                    annotation_path = os.path.join(args.train_data_dir, "sekai_train_10k", "vstreams_moderate_clip_annotation", '_'.join(validation_id.split('_')[:-4]), validation_id)

                    gt_video_path = os.path.join(args.train_data_dir, "sekai_train_10k", "vstreams_moderate_clip", '_'.join(validation_id.split('_')[:-4]), f"{validation_id}.mp4")

                    if os.path.isdir(annotation_path):
                        gt_video, _, _, _ = get_video_to_video_latent(gt_video_path, video_length=video_length, sample_size=args.video_sample_size)


                        # Multi-control case: load multiple control videos
                        control_filenames = ["background_RGB.mp4", "background_depth.mp4", "3D_gaussian_RGB.mp4", "3D_gaussian_depth.mp4"]
                        control_videos = []
                        
                        for control_filename in control_filenames:
                            control_video_path = os.path.join(annotation_path, control_filename)
                            if os.path.exists(control_video_path):
                                input_video, _, _, _ = get_video_to_video_latent(control_video_path, video_length=video_length, sample_size=args.video_sample_size)
                                control_videos.append(input_video)
                            else:
                                logger.warning(f"Control video not found: {control_video_path}")
                                # Create a zero tensor as placeholder
                                if len(control_videos) > 0:
                                    control_videos.append(torch.zeros_like(control_videos[0]))
                        
                        # Load mask
                        mask_path = os.path.join(annotation_path, "merged_mask.mp4")
                        if os.path.exists(mask_path):
                            input_video_mask, _, _, _ = get_video_to_video_latent(mask_path, video_length=video_length, sample_size=args.video_sample_size)
                            input_video_mask = input_video_mask[:, :1]
                            input_video_mask[:,:,0] = 0.0
                        else:
                            input_video_mask = torch.ones_like(control_videos[0][:, :1]) * 255
                        
                        # Load reference image (first frame from background_rgb)
                        ref_image = gt_video[:, :, :1]
                        control_videos[0][:,:,0] = gt_video[:, :, 0]
                        
                    else:
                        # Single control case (backward compatibility)
                        input_video, input_video_mask, ref_image, clip_image = get_video_to_video_latent(annotation_path, video_length=video_length, sample_size=args.video_sample_size)
                        control_videos = input_video
            ...
            print(f'Generated video: {os.path.join(args.output_dir, f"sample/sample-{global_step}-{i}.mp4")}')
            ...
            for tracker in accelerator.trackers:
                if tracker.name == "wandb":
                    tracker.log(
                        {
                            "sample": wandb.Video(
                                os.path.join(args.output_dir, f"sample/sample-{global_step}-{i}.mp4"), 
                                caption=f"sample-{global_step}-{i}")
                        }
                    )

        transformer3d_unwrapped.train()
        ...
        try:
            if 'transformer3d_unwrapped' in locals():
                transformer3d_unwrapped.train()
        except:
            pass
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="+",
        help=("A set of paths to validation data evaluated every `--validation_epochs` and logged to `--report_to`."),
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help=("The input dimension for the GeoAda patch embedding. If specified, it will override the config value."),
    )
...
def main():
    ...
    if args.geoada_in_dim is not None:
        ...
        print(f"Overriding geoada_in_dim with command line argument: {args.geoada_in_dim}")
    
    transformer3d = VerseCrafterWanTransformer3DModel.from_pretrained(
    ...
    if args.use_ema:
        ...
        ema_transformer3d = VerseCrafterWanTransformer3DModel.from_pretrained(
            os.path.join(args.pretrained_model_name_or_path, config['transformer_additional_kwargs'].get('transformer_subpath', 'transformer')),
            transformer_additional_kwargs=transformer_additional_kwargs,
        ).to(weight_dtype)

        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=VerseCrafterWanTransformer3DModel, model_config=ema_transformer3d.config)
        ...
        if fsdp_stage != 0:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = VerseCrafterWanTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
                    load_model = VerseCrafterWanTransformer3DModel.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=OmegaConf.to_container(config['transformer_additional_kwargs'])
                    )
                    load_model = EMAModel(load_model.parameters(), model_cls=VerseCrafterWanTransformer3DModel, model_config=load_model.config)
    ...
    # Get the dataset
    train_dataset = VideoControlDataset_sekai_spatialvid(
    ...
    if args.enable_bucket:
        ...
        def collate_fn(examples):
            ...
            target_size = [int(x) for x in (fix_sample_size if args.fix_sample_size is not None else closest_size)]
            ...
            for example in examples:
                ...
                # ref_pixel_values = padding_image(ref_pixel_values, target_size[1], target_size[0])
                ref_pixel_values = ref_pixel_values.resize((target_size[1], target_size[0]))
                ...
                num_subject = 0 if example["subject_image"] is None else len(example["subject_image"])
                if num_subject != 0:
    ...
    if accelerator.is_main_process:
        ...
        keys_to_remove = [k for k, v in tracker_config.items() if v is None or isinstance(v, list)]
        for key in keys_to_remove:
            tracker_config.pop(key)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                background_rgb_values = batch["background_rgb"].cpu()
                background_depth_bw_values = batch["background_depth_bw"].cpu()
                gaussian_projection_values = batch["gaussian_projection"].cpu()
                foreground_ellipsoid_depth_bw_values = batch["foreground_ellipsoid_depth_bw"].cpu()
                mask_pixel_values = batch['mask_pixel_values'].cpu()
                file_names = batch['file_name']

                pixel_values = rearrange(pixel_values, "b f c h w -> b c f h w")
                background_rgb_values = rearrange(background_rgb_values, "b f c h w -> b c f h w")
                background_depth_bw_values = rearrange(background_depth_bw_values, "b f c h w -> b c f h w")
                gaussian_projection_values = rearrange(gaussian_projection_values, "b f c h w -> b c f h w")
                foreground_ellipsoid_depth_bw_values = rearrange(foreground_ellipsoid_depth_bw_values, "b f c h w -> b c f h w")
                mask_pixel_values = rearrange(mask_pixel_values, "b f c h w -> b c f h w")

                os.makedirs(os.path.join(args.output_dir, "sanity_check"), exist_ok=True)
                for idx, (pixel_value, background_rgb_value, background_depth_bw_value, gaussian_projection_value, foreground_ellipsoid_depth_bw_value, mask_pixel_value, file_name) in enumerate(zip(pixel_values, background_rgb_values, background_depth_bw_values, gaussian_projection_values, foreground_ellipsoid_depth_bw_values, mask_pixel_values, file_names)):
                    pixel_value = pixel_value[None, ...]
                    background_rgb_value = background_rgb_value[None, ...]
                    background_depth_bw_value = background_depth_bw_value[None, ...]
                    gaussian_projection_value = gaussian_projection_value[None, ...]
                    foreground_ellipsoid_depth_bw_value = foreground_ellipsoid_depth_bw_value[None, ...]
                    mask_pixel_value = mask_pixel_value[None, ...]
                    gif_name = file_name.split('.')[0] if not file_name == '' else f'{global_step}-{idx}'

                    save_videos_grid(pixel_value, f"{args.output_dir}/sanity_check/{gif_name}.mp4", rescale=True)
                    save_videos_grid(background_rgb_value, f"{args.output_dir}/sanity_check/{gif_name}_background_RGB.mp4", rescale=True)
                    save_videos_grid(background_depth_bw_value, f"{args.output_dir}/sanity_check/{gif_name}_background_depth.mp4", rescale=True)
                    save_videos_grid(gaussian_projection_value, f"{args.output_dir}/sanity_check/{gif_name}_3D_gaussian_RGB.mp4", rescale=True)
                    save_videos_grid(foreground_ellipsoid_depth_bw_value, f"{args.output_dir}/sanity_check/{gif_name}_3D_gaussian_depth.mp4", rescale=True)
                    save_videos_grid(mask_pixel_value, f"{args.output_dir}/sanity_check/{gif_name}_merged_mask.mp4", rescale=False)

            with accelerator.accumulate(transformer3d):
                # Convert images to latent space
                pixel_values = batch["pixel_values"].to(weight_dtype)
                background_rgb_values = batch["background_rgb"].to(weight_dtype)
                background_depth_bw_values = batch["background_depth_bw"].to(weight_dtype)
                gaussian_projection_values = batch["gaussian_projection"].to(weight_dtype)
                foreground_ellipsoid_depth_bw_values = batch["foreground_ellipsoid_depth_bw"].to(weight_dtype)
                ...
                def geoada_encode_multi_frames(multi_frames, ref_images):
                    weight_dtype = multi_frames[0].dtype
                    if ref_images is None:
                        ref_images = [None] * len(multi_frames[0])
                    else:
                        assert len(multi_frames[0]) == len(ref_images)

                    encoded_latents = []
                    for frames in multi_frames:
                        latent = vae.encode(frames)[0].mode()
                        encoded_latents.append(latent)
                    
                    # Concatenate each latent (similar to inactive and reactive)
                    # latents = [torch.cat(items, dim=0) for items in zip(*encoded_latents)]
                ...
                def geoada_encode_frames(frames, ref_images, masks=None):
                ...
                def geoada_encode_masks(masks, ref_images=None, vae_stride=[4, 8, 8]):
                    if ref_images is None:
                ...
                with torch.no_grad():
                    ...
                    if inpaint_flag or (background_rgb_values == -1).all():
                        if rng is None:
                        ...
                        if do_not_use_ref_images:
                            subject_ref_images = None
                        mask_pixel_values = rearrange(mask_pixel_values, "b f c h w -> b c f h w")
                        geoada_latents = geoada_encode_frames(mask_pixel_values, subject_ref_images, mask)
                    else:
                        background_rgb_values = rearrange(background_rgb_values, "b f c h w -> b c f h w")
                        background_depth_bw_values = rearrange(background_depth_bw_values, "b f c h w -> b c f h w")
                        gaussian_projection_values = rearrange(gaussian_projection_values, "b f c h w -> b c f h w")
                        foreground_ellipsoid_depth_bw_values = rearrange(foreground_ellipsoid_depth_bw_values, "b f c h w -> b c f h w")
                        mask_pixel_values = rearrange(mask_pixel_values, "b f c h w -> b c f h w")
                        geoada_latents = geoada_encode_multi_frames([background_rgb_values, background_depth_bw_values, gaussian_projection_values, foreground_ellipsoid_depth_bw_values], subject_ref_images)
                        # mask = torch.ones_like(mask)

                    mask_latents = geoada_encode_masks(mask_pixel_values, subject_ref_images)
                    geoada_context = torch.stack(geoada_latent(geoada_latents, mask_latents))
                ...
                target_shape = (vae.latent_channels, geoada_latents[0].size(1), width, height)
                seq_len = math.ceil(
